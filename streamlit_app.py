...
def main():
    ...
    with col2_1:
        ...
        fig_vix.update_layout(title="恐慌結構：VIX / VIX3M 比率", height=350)
    ...
    with col2_3:
        ...
        fig_risk.update_layout(title="風險偏好：HYG / TLT 比率", height=350)
    ...
    # 象限標籤
    fig_rrg.add_annotation(x=2, y=2, text="領先", showarrow=False, font=dict(size=16, color="green"), opacity=0.5)
    fig_rrg.add_annotation(x=2, y=-2, text="轉弱", showarrow=False, font=dict(size=16, color="orange"), opacity=0.5)
    fig_rrg.add_annotation(x=-2, y=-2, text="落後", showarrow=False, font=dict(size=16, color="red"), opacity=0.5)
    fig_rrg.add_annotation(x=-2, y=2, text="改善", showarrow=False, font=dict(size=16, color="blue"), opacity=0.5)
    ...
    with col3:
        ...
        gainer_df['Val'] = gainer_df['RVol'] # 使用 RVol 作為 Val 顯示
